...
function worldMap(data) {
     ...
     */
    // I index.html heter ID elementet för "mapid" därför fungerade inte "map"
    var Leaflet_map = L.map("mapid").setView([10, 15], 1);

    /**
     * Task 15 - Get the tileLayer from the link at the bottom of this file
    */
    titleLayer = L.tileLayer(map_link()).addTo(Leaflet_map);
    ...
    function projectPointsOnMap(x, y) {
        // Converts geographic coordinates to pixel positions on the map
        var point = Leaflet_map.latLngToLayerPoint(new L.LatLng(y, x));
        // Uses these pixel positions above to draw a point on an SVG representation of the map using D3s stream.point method
